...
async function certificate(req, res, next) {
  try {
    ...
    const grade = await prisma.academic_grades.findUnique({
      where: { module_id_user_id_academic_periods: { module: 'vinculacion', id_user: Number(studentId), id_academic_periods: Number(periodo_id) } },
      select: { score: true }
    }).catch(() => null);
    ...
    const uploadsDir = path.join(process.cwd(), 'uploads', 'certificados', 'vinculacion', String(studentId));
    fs.mkdirSync(uploadsDir, { recursive: true });
    const filename = `cert_vinculacion_${studentId}_${Date.now()}.pdf`;
    ...
    await new Promise((resolve, reject) => {
      ...
      doc.y = 95;
      doc.font('Helvetica-Bold').fontSize(14).text('CERTIFICADO DE VINCULACIÓN CON LA SOCIEDAD', { align: 'center' });
      doc.moveDown(1.2);

      doc.font('Helvetica').fontSize(11);
      const p1 = `El/la estudiante ${estudianteNombre}, con número de identificación ${estudianteCedula}, de conformidad con la Ley Orgánica de Educación Superior y su Reglamento, ha cumplido con los requisitos respectivos del programa de Vinculación con la Sociedad, completando las actividades y el número de horas establecidas, según consta en los archivos que reposan en la Coordinación de Vinculación, a los que me remitiré en caso de ser necesario; por lo tanto, se deja constancia del cumplimiento de este requisito académico.`;
      ...
      doc.font('Helvetica-Bold').fontSize(10).text('VINCULACIÓN CON LA SOCIEDAD', sigX, doc.y, { width: sigW, align: 'center' });
...
async function recientes(req, res, next) {
  try {
    ...
    const { days, academicPeriodId } = schema.parse(req.query || {});
    const id_ap = await getActiveAcademicPeriodId(academicPeriodId);
    if (!Number.isFinite(Number(id_ap))) return res.json([]);

    const since = Number.isFinite(Number(days))
